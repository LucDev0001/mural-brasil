import { db, auth, onAuthStateChanged, collection, query, where, getDocs, doc, updateDoc, deleteDoc } from "./firebase-init.js";
...
async function loadPendingSubmissions() {
  ...
  pendingSubmissionsContainer.innerHTML = '<p class="text-center">Carregando submissões pendentes...</p>';

  try {
    const q = query(collection(db, "submissoes"), where("status", "==", "pendente"));
    const querySnapshot = await getDocs(q);

    if (querySnapshot.empty) {
      pendingSubmissionsContainer.innerHTML = '<p class="text-center">Nenhuma submissão pendente.</p>';
      return;
    }

    const submissionsHTML = querySnapshot.docs.map(doc => createSubmissionCard(doc)).join('');
    pendingSubmissionsContainer.innerHTML = submissionsHTML;

    // Add event listeners after rendering the buttons
    querySnapshot.docs.forEach(doc => {
        const approveBtn = document.querySelector(`[data-id="${doc.id}"].approve-btn`);
        const rejectBtn = document.querySelector(`[data-id="${doc.id}"].reject-btn`);
    ...
    pendingSubmissionsContainer.innerHTML = '<p class="text-center text-red-500">Erro ao carregar submissões.</p>';
